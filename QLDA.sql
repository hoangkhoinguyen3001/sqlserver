...
SELECT NV1.HO + ' ' + NV1.TENLOT + ' ' + NV1.TEN AS HOTENNV, COUNT(NV2.MANV_QUANLY) AS SONV
FROM NHANVIEN NV1 LEFT JOIN NHANVIEN NV2
ON NV1.MANV = NV2.MANV_QUANLY
GROUP BY NV1.HO + ' ' + NV1.TENLOT + ' ' + NV1.TEN
--CAU 28: Với mỗi phòng ban, liệt kê tên phòng ban và lương trung bình của những nhân viên làm việc tại phòng ban đó
...
GROUP BY TENPHONG
--CAU 30:
SELECT TENPHONG,COUNT(DA.MAPHONG) AS SLDA
FROM PHONGBAN PB INNER JOIN DEAN D on PB.MAPHONG = D.MAPHONG
GROUP BY TENPHONG
--CAU 31:
SELECT TENPHONG,HO + ' ' + TENLOT + ' ' + TEN AS HOTENTRUONG, COUNT(D.MAPHONG) AS SLDA
FROM NHANVIEN NV INNER JOIN PHONGBAN P on NV.MAPHONG = P.MAPHONG
                 INNER JOIN DEAN D on NV.MAPHONG = D.MAPHONG
WHERE NV.MANV = P.MANV_TRUONGPHONG
GROUP BY TENPHONG,HO + ' ' + TENLOT + ' ' + TEN
--CAU 32:
SELECT TENPHONG,COUNT(D.MAPHONG),AVG(LUONG)
FROM PHONGBAN PB INNER JOIN DEAN D on PB.MAPHONG = D.MAPHONG
                 INNER JOIN NHANVIEN N on D.MAPHONG = N.MAPHONG
GROUP BY TENPHONG
HAVING AVG(LUONG) > 40000
--CAU 33:
SELECT DIADIEM,COUNT(TENDA)
FROM DEAN
GROUP BY DIADIEM
--CAU 34:
SELECT TENDA,COUNT(C.MADA)
FROM DEAN DA INNER JOIN CONGVIEC C on DA.MADA = C.MADA
GROUP BY TENDA
--CAU 35:
SELECT COUNT(P.MADA) AS SL
FROM DEAN DA INNER JOIN PHANCONG P on DA.MADA = P.MADA
WHERE P.MADA = '30'
GROUP BY P.MADA
--CAU 36:
...
GROUP BY P.MADA
--CAU 37:
SELECT PHANCONG.MADA,NHANVIEN.HO
FROM NHANVIEN,PHANCONG
WHERE NHANVIEN.MANV = PHANCONG.MANV
          AND  (NHANVIEN.HO =N'Phạm' or NHANVIEN.HO = N'Đinh' )
--CAU 38:
SELECT HO + ' ' + TENLOT + ' ' + TEN AS HOTEN,COUNT(T.MANV)
FROM NHANVIEN NV INNER JOIN THANNHAN T on NV.MANV = T.MANV
GROUP BY HO + ' ' + TENLOT + ' ' + TEN
HAVING COUNT(T.MANV) > 1
--CAU 39:
SELECT HO + ' ' + TENLOT + ' ' + TEN AS HOTEN,COUNT(T.MANV)
FROM NHANVIEN NV LEFT JOIN THANNHAN T on NV.MANV = T.MANV
GROUP BY HO + ' ' + TENLOT + ' ' + TEN
HAVING COUNT(T.MANV) < 1
--CAU 40:
SELECT HO + ' ' + TENLOT + ' ' + TEN AS HOTENTRUONGPHONG
FROM NHANVIEN
WHERE MANV IN  (
    SELECT MANV_TRUONGPHONG
    FROM PHONGBAN
    WHERE MANV_TRUONGPHONG = NHANVIEN.MANV)
    AND MANV IN (
        SELECT MANV
        FROM THANNHAN
...
WHERE MANV IN  (
    ...
    AND MANV NOT IN (
        ...
        WHERE NHANVIEN.MANV =  THANNHAN.MANV  )
--CAU 42:
SELECT HO + ' ' + TENLOT + ' ' + TEN AS HOTENNV,LUONG
FROM NHANVIEN
WHERE LUONG IN (
    SELECT AVG(LUONG) AS LUONGTRENTB
...
--CAU 43:
SELECT TENPHONG,HO + ' ' + TENLOT + ' ' + TEN AS TRUONGPHONG
FROM NHANVIEN NV INNER JOIN PHONGBAN P on NV.MAPHONG = P.MAPHONG
WHERE NV.MANV = P.MAPHONG AND NV.MAPHONG = (
    SELECT MAX(MAPHONG) AS PHONGNHIEUNV
    FROM NHANVIEN
     WHERE MAPHONG = MANV_QUANLY
    GROUP BY MAPHONG
)
